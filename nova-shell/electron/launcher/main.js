...
function createLauncherWindow() {
  ...
  const startHidden = process.env.NOVASHELL_LAUNCHER_START_HIDDEN === '1';
  if (!startHidden) {
...
app.setName('nova-shell-launcher');

app.whenReady().then(() => {
  createLauncherWindow();

  ipcMain.handle('novaShell:getApps', () => {
    return loadDesktopEntries();
  });

  ipcMain.handle('novaShell:launch', (_event, appEntry) => {
    launchApp(appEntry);
